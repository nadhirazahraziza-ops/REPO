...
// center: koordinat pusat peta [longitude, latitude] Yogyakarta.
// zoom: tingkat kedekatan peta.
const map = new maplibregl.Map({
  container: "map",
  style: BASEMAP,
  center: [110.3670, -7.8014],
  zoom: 11
});
...
// Konfigurasi data GeoJSON dan proyeksi sumber.
const GEOJSON_PATH = "./data/Pneumonia%20yogya.geojson";
const SOURCE_CRS = "+proj=utm +zone=49 +south +datum=WGS84 +units=m +no_defs";
const DEST_CRS = "EPSG:4326";

// Nama source dan layer dibuat sebagai konstanta agar tidak salah ketik.
const SOURCE_ID = "pneumonia-yogya-source";
const FILL_LAYER_ID = "pneumonia-yogya-fill";
const LINE_LAYER_ID = "pneumonia-yogya-outline";

// ===============================
// 3. Membaca file GeoJSON lokal
// ===============================
// File berada di folder data, sehingga path-nya ./data/Pneumonia yogya.geojson
async function loadGeoJSON() {
  try {
    ...
    pendudukData = reprojectGeoJSON(await response.json());
...
function reprojectCoordinates(coords) {
  if (typeof coords[0] === "number" && typeof coords[1] === "number") {
    const [x, y, z] = coords;
    const [lon, lat] = proj4(SOURCE_CRS, DEST_CRS, [x, y]);
...
function addPendudukLayer() {
  ...
  // Layer fill untuk menampilkan polygon kecamatan.
  map.addLayer({
    id: FILL_LAYER_ID,
    type: "fill",
    source: SOURCE_ID,
    paint: {
      // Warna polygon dibuat berdasarkan atribut jumlah_kas.
      // Semakin besar jumlah kasus, semakin gelap warnanya.
      "fill-color": [
        "interpolate",
        ["linear"],
        ["to-number", ["get", "jumlah_kas"]],
        0, "#fff7bc",
        20, "#fec44f",
        40, "#d95f0e",
        60, "#7f2704"
...
function setupPopup() {
  ...
  map.on("click", FILL_LAYER_ID, (event) => {
    ...
    const popupHTML = `
      <div>
        <div class="popup-title">${props.wadmkc || "Tanpa Nama"}</div>
        <div class="popup-row"><b>Kecamatan:</b> ${props.wadmkc || "-"}</div>
        <div class="popup-row"><b>Jumlah Kasus:</b> ${Number(props.jumlah_kas || 0).toLocaleString("id-ID")} kasus</div>
        <div class="popup-row"><b>Kota:</b> ${props.wadmkk || "-"}</div>
      </div>
    `;
